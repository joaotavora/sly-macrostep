...
;; Tests for sly-macrostep.  The following are expected failures:
...
;;   - it currently lacks a working SLY defimplementation for
...
(defun sly-macrostep-eval-definitions (definitions)
  (sly-check-top-level)
  (sly-compile-string definitions 0)
  (sly-sync-to-top-level 5))

(defmacro sly-macrostep-with-text (buffer-text &rest body)
  (declare (indent 1))
  `(with-temp-buffer
     (lisp-mode)
     (save-excursion
...
(defun sly-macrostep-search (form)
...
(def-sly-test (sly-macrostep-expand-defmacro)
  ...
  (sly-macrostep-eval-definitions definition)
  (sly-macrostep-with-text buffer-text
    (sly-macrostep-search original)
    (macrostep-expand)
    (sly-test-expect "Macroexpansion is correct"
                       expansion
                       (downcase (sly-sexp-at-point))
                       #'sly-test-macroexpansion=)))

(def-sly-test (sly-macrostep-fontify-macros
                 (:fails-for "clisp" "ECL"))
  ...
  (sly-macrostep-with-text buffer-text
    (sly-macrostep-search original)
    (macrostep-expand)
    (sly-macrostep-search subform)
    (forward-char)                      ; move over open paren
    (sly-check "Head of macro form in expansion is fontified correctly"
        (eq (get-char-property (point) 'font-lock-face)
         'macrostep-macro-face))))

(def-sly-test (sly-macrostep-fontify-compiler-macros
  ...
  (sly-macrostep-with-text buffer-text
    ...
    (sly-macrostep-search subform)
    (forward-char)                      ; move over open paren
    (sly-check "Head of compiler-macro in expansion is fontified correctly"
        (eq (get-char-property (point) 'font-lock-face)
         'macrostep-compiler-macro-face))))

(def-sly-test (sly-macrostep-expand-macrolet
                 (:fails-for "ECL"))
  ...
  (when definitions
    (sly-macrostep-eval-definitions definitions))
  (sly-macrostep-with-text buffer-text
    ;; sly-test-macroexpansion= does not expect tab characters,
...
(def-sly-test (sly-macrostep-fontify-local-macros
                 (:fails-for "clisp" "ECL"))
    ()
    "Test that locally-bound macros are highlighted in expansions."
    '(())
    (sly-macrostep-with-text
...
(def-sly-test (sly-macrostep-handle-unreadable-objects)
